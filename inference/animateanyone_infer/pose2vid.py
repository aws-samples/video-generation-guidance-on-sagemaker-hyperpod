...
def main():
    ...
    infer_config = config.inference_config
    ...
    save_dir = Path(f"{args.save_dir}/{date_str}/{save_dir_name}")
    ...
    for ref_image_path in config["test_cases"].keys():
        ...
        for pose_video_path in config["test_cases"][ref_image_path]:
            ...
            save_path = f"{save_dir}/{ref_name}_{pose_name}_{args.H}x{args.W}_{int(args.cfg)}_{time_str}.mp4"
            save_videos_grid(
                video,
                save_path,
                n_rows=3,
